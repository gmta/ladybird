...
class ReportsHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        signal_name = re.search(r"Termination signal name: (\w+)", attachment_bytes.decode("utf-8"))
        signal_number = re.search(r"Termination signal number: (\d+)", attachment_bytes.decode("utf-8"))
        assert signal_name is not None and signal_number is not None
        assert fields["signal"] == {"type": "text", "value": signal_name.group(1)}
        assert fields["signal_number"] == {"type": "number", "value": int(signal_number.group(1))}
