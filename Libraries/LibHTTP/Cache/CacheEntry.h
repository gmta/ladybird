...
struct CacheFileHeader {
    ...
    u64 cache_key { 0 };
    u64 vary_key { 0 };
    Crypto::Hash::SHA1::DigestType body_hash {};
    u32 header_hash { 0 };
};
...
// on disk is:
//
//     [CacheFileHeader][Data]
class CacheEntry {
public:
    static constexpr u64 DATA_OFFSET = sizeof(CacheFileHeader);
...
protected:
    CacheEntry(DiskCache&, CacheIndex&, u64 cache_key, u64 vary_key, String url, Optional<LexicalPath>);
...
private:
    CacheEntryWriter(DiskCache&, CacheIndex&, u64 cache_key, String url, UnixDateTime request_time, AK::Duration current_time_offset_for_testing);
    ...
    u32 m_status_code { 0 };
    u64 m_data_size { 0 };
    OwnPtr<Crypto::Hash::SHA1> m_body_hasher;
};
