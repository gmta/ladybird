...
void Application::initialize_actions()
{
    ...
    m_debug_menu->add_action(Action::create("Dump Page Snapshot ZIP"sv, ActionID::DumpPageSnapshot, [this]() {
        if (auto view = active_web_view(); view.has_value()) {
            auto snapshot_path = view->dump_page_snapshot_zip();
            if (snapshot_path.is_error())
                warnln("\033[31;1mFailed to dump page snapshot ZIP: {}\033[0m", snapshot_path.error());
        }
    }));
