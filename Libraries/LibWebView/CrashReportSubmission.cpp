...
void CrashReportSubmission::request_challenge(ByteString manifest)
{
    ...
    post(challenge_path, "application/json"sv, body.serialized().to_byte_string(), {},
        [this, manifest = move(manifest)](u32 status, ByteString response) mutable {
            if (status != 200) {
                fail(MUST(String::formatted("Report server returned {} while requesting a challenge.", status)));
                return;
            }

            auto challenge = parse_json_object(response);
            if (!challenge.has_value()) {
                fail("The report server sent an unreadable challenge."_string);
                return;
            }

            auto algorithm = challenge->get_string("algorithm"sv);
            if (!algorithm.has_value() || *algorithm != challenge_algorithm) {
                fail("Unsupported report challenge."_string);
                return;
            }
            ...
            if (!token.has_value() || token->bytes().size() > maximum_challenge_token_length
                ...
                fail("The report server supplied an invalid challenge."_string);
                return;
            }
            ...
            main_thread_event_loop.deferred_invoke(
                [weak_this = move(weak_this), attempt, manifest = move(manifest), token = move(token),
                    ...
                    if (!self || self->m_attempt != attempt)
                        return;
                    if (!nonce.has_value()) {
                        self->fail("Could not compute the report's proof of work."_string);
                        return;
                    }
...
void CrashReportSubmission::send_report(ByteString manifest, ProofOfWork proof)
{
    ...
    post(report_path, content_type, move(body), move(proof), [this](u32 status, ByteString response) {
        if (status < 200 || status >= 300) {
            fail(MUST(String::formatted("Report server returned {} while sending the report.", status)));
            return;
        }
        ...
        if (!receipt.has_value() || receipt->is_empty()) {
            fail("Report server did not provide a receipt."_string);
            return;
        }
        finish();
    });
...
void CrashReportSubmission::post(StringView path, StringView content_type, ByteString body,
    Optional<ProofOfWork> proof, Function<void(u32, ByteString)> on_response)
{
    ...
    if (!url.has_value() || (url->scheme() != "https"sv && url->scheme() != "http"sv)) {
        fail("Invalid report server configuration."_string);
        return;
    }
    ...
    if (!m_request) {
        fail("Could not connect to the report server."_string);
        return;
    }
    ...
    m_request_timeout = Core::Timer::create_single_shot(request_timeout_ms,
        [weak_this = make_weak_ptr<CrashReportSubmission>()] {
            ...
            if (request)
                request->stop();
            self->fail("The report server did not respond in time."_string);
        });
    m_request_timeout->start();

    m_request->set_buffered_request_finished_callback(
        [weak_this = make_weak_ptr<CrashReportSubmission>(), on_response = move(on_response)](u64,
            Requests::RequestTimingInfo const&, Optional<Requests::NetworkError> const& network_error,
            HTTP::HeaderList const&, Optional<u32> response_code, Optional<String> const&,
            ...
            if (network_error.has_value() || !response_code.has_value()) {
                self->fail("Network error while contacting the report server."_string);
                return;
            }
            on_response(*response_code, ByteString::copy(payload.bytes()));
        });
}
...
void CrashReportSubmission::fail(String reason)
{
    ...
    if (on_retry)
        on_retry(move(reason), retry_number);

    m_retry_timer = Core::Timer::create_single_shot(retry_delay_ms,
        [weak_this = make_weak_ptr<CrashReportSubmission>()] {
            if (auto self = weak_this.strong_ref())
                self->begin_attempt();
        });
