...
Color highlight_text(PreferredColorScheme scheme)
{
    ...
    // When the values of <system-color> keywords come from the browser, (as opposed to being OS defaults or user
    // choices) the browser should ensure that matching foreground/background pairs have a minimum of WCAG AA contrast.
    // NB: Since Highlight is translucent in Ladybird, choose the foreground color based on Highlight composited over
    // Canvas for the current color scheme.
    return canvas(scheme).blend(highlight(scheme)).suggested_foreground_color();
}
