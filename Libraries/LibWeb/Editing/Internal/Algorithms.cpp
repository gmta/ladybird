...
void canonicalize_whitespace(DOM::Node& node, u32 offset, bool fix_collapsed_space)
{
    ...
    while (true) {
        ...
        if (layout_node && start_node->is_text() && start_offset != 0) {
    ...
    while (true) {
        ...
        if (layout_node && end_node->is_text() && end_offset != end_node->length()) {
            auto parent_white_space = layout_node->computed_values().white_space();
    ...
    if (fix_collapsed_space) {
        while (true) {
            ...
            //    break:
            auto* layout_node = end_node->parent()->layout_node();
            if (layout_node && end_node->is_text() && end_offset == end_node->length() && precedes_a_line_break(end_node)) {
                auto parent_white_space = layout_node->computed_values().white_space();
    ...
    while (true) {
        ...
        else if (!start_node->is_text() || start_offset == start_node->length()) {
...
DOM::Node const* editing_host_of_node(DOM::Node const& node)
{
...
bool is_allowed_child_of_node(DOM::Node const&, Variant<DOM::Node*, String>)
{
    // FIXME: 1. If parent is "colgroup", "table", "tbody", "tfoot", "thead", "tr", or an HTML element with local name equal to
    //    one of those, and child is a Text node whose data does not consist solely of space characters, return false.

    // FIXME: 2. If parent is "script", "style", "plaintext", or "xmp", or an HTML element with local name equal to one of
    //    those, and child is not a Text node, return false.

    // FIXME: 3. If child is a document, DocumentFragment, or DocumentType, return false.

    // FIXME: 4. If child is an HTML element, set child to the local name of child.

    // FIXME: 5. If child is not a string, return true.

    // FIXME: 6. If parent is an HTML element:
    {
        // FIXME: 1. If child is "a", and parent or some ancestor of parent is an a, return false.

        // FIXME: 2. If child is a prohibited paragraph child name and parent or some ancestor of parent is an element with
        //    inline contents, return false.

        // FIXME: 3. If child is "h1", "h2", "h3", "h4", "h5", or "h6", and parent or some ancestor of parent is an HTML
        //    element with local name "h1", "h2", "h3", "h4", "h5", or "h6", return false.

        // FIXME: 4. Let parent be the local name of parent.
    }

    // FIXME: 7. If parent is an Element or DocumentFragment, return true.

    // FIXME: 8. If parent is not a string, return false.

    // FIXME: 9. If parent is on the left-hand side of an entry on the following list, then return true if child is listed on
    ...
    // FIXME: 10. If child is "body", "caption", "col", "colgroup", "frame", "frameset", "head", "html", "tbody", "td",
    //     "tfoot", "th", "thead", or "tr", return false.

    // FIXME: 11. If child is "dd" or "dt" and parent is not "dl", return false.

    // FIXME: 12. If child is "li" and parent is not "ol" or "ul", return false.

    // FIXME: 13. If parent is on the left-hand side of an entry on the following list and child is listed on the right-hand
...
bool is_block_boundary_point(DOM::Node const& node, u32 offset)
{
...
bool is_block_end_point(DOM::Node const& node, u32 offset)
{
...
bool is_block_node(DOM::Node const& node)
{
    ...
    if (node.is_document() || node.is_document_fragment())
        return true;
...
bool is_block_start_point(DOM::Node const& node, u32 offset)
{
    ...
    return is_visible_node(*offset_minus_one_child)
        && (is_block_node(*offset_minus_one_child) || offset_minus_one_child->is_html_br_element());
}

// https://w3c.github.io/editing/docs/execCommand/#collapsed-whitespace-node
bool is_collapsed_whitespace_node(DOM::Node const& node)
{
    ...
    if (node.is_character_data()) {
        auto const& character_data = dynamic_cast<DOM::CharacterData const&>(node);
    ...
    while (reference->is_descendant_of(*ancestor)) {
        ...
        if (is_block_node(*reference) || reference->is_html_br_element())
            return true;

        // 3. If reference is a Text node that is not a whitespace node, or is an img, break from
        //    this loop.
        if ((reference->is_text() && !is_whitespace_node(*reference)) || reference->is_html_image_element())
            break;
    }
...
bool is_editing_host(DOM::Node const& node)
{
...
bool is_extraneous_line_break(DOM::Node const& node)
{
    // An extraneous line break is a br
    if (!node.is_html_br_element())
        return false;

    // ...except that a br that is the sole child of an li is not extraneous.
    auto parent = node.parent();
    if (parent && static_cast<DOM::Element const*>(parent)->local_name() == HTML::TagNames::li && parent->child_count() == 1)
        return false;
...
bool is_in_same_editing_host(DOM::Node const& node_a, DOM::Node const& node_b)
{
...
bool is_inline_node(DOM::Node const& node)
{
...
bool is_invisible_node(DOM::Node const& node)
{
...
bool is_visible_node(DOM::Node const& node)
{
    ...
    if (node.is_text() && !is_collapsed_whitespace_node(node))
        return true;

    // or an img,
    if (node.is_html_image_element())
        return true;

    // or a br that is not an extraneous line break,
    if (node.is_html_br_element() && !is_extraneous_line_break(node))
        return true;

    // or any node with a visible descendant;
    // AD-HOC: We call into is_visible_node() recursively, so check children instead of descendants.
    bool has_visible_child_node = false;
    node.for_each_child([&](DOM::Node const& child_node) {
...
bool is_whitespace_node(DOM::Node const& node)
{
    // NOTE: All constraints below check that node is a Text node with data
    if (!node.is_text() || !node.is_character_data())
        return false;
...
void move_node_preserving_ranges(DOM::Node& node, DOM::Node& new_parent, u32 new_index)
{
...
void normalize_sublists_in_node(DOM::Node& item)
{
    // 1. If item is not an li or it is not editable or its parent is not editable, abort these
    //    steps.
    auto& item_element = static_cast<DOM::Element&>(item);
    if (item_element.local_name() != HTML::TagNames::li || !item_element.is_editable() || !item_element.parent()->is_editable())
        return;
    ...
    while (item.has_child_of_type<HTML::HTMLOListElement>() || item.has_child_of_type<HTML::HTMLUListElement>()) {
        ...
        if ((is<HTML::HTMLOListElement>(child) || is<HTML::HTMLUListElement>(child))
            || (!new_item && child.is_text() && text_is_all_whitespace)) {
...
bool precedes_a_line_break(DOM::Node const* node)
{
...
Vector<RecordedNodeValue> record_the_values_of_nodes(Vector<DOM::Node*> const& node_list)
{
    ...
    for (auto* node : node_list) {
        for (auto command : commands) {
            ...
            //    it to its parent.
            while (ancestor->is_element() && !specified_command_value(*static_cast<DOM::Element*>(ancestor), command).has_value())
                ancestor = ancestor->parent();
...
void remove_extraneous_line_breaks_at_the_end_of_node(DOM::Node& node)
{
    // 1. Let ref be node.
    auto* ref = &node;
...
void remove_extraneous_line_breaks_before_node(DOM::Node& node)
{
    // 1. Let ref be the previousSibling of node.
    auto* ref = node.previous_sibling();
...
void remove_extraneous_line_breaks_from_a_node(DOM::Node& node)
{
...
void restore_the_values_of_nodes(Vector<RecordedNodeValue> const& values)
{
    ...
    for (auto& recorded_node_value : values) {
        // 1. Let ancestor equal node.
        auto* ancestor = &recorded_node_value.node;
...
Optional<String> specified_command_value(DOM::Element const& element, FlyString const& command)
{
...
void split_the_parent_of_nodes(Vector<DOM::Node*> const& nodes)
{
    VERIFY(nodes.size() > 0);

    // 1. Let original parent be the parent of the first member of node list.
    auto& first_node = *nodes.first();
    auto& last_node = *nodes.last();
    auto& original_parent = *first_node.parent();
    ...
    auto* first_child = original_parent.first_child();
    ...
    auto* last_child = original_parent.last_child();
    ...
    auto& parent_of_original_parent = *original_parent.parent();
    ...
    if (!first_child_in_nodes_list) {
        ...
        while (first_node.previous_sibling() != nullptr)
            move_node_preserving_ranges(*original_parent.first_child(), cloned_parent, cloned_parent->child_count() - 1);
    }
    ...
    if (is_inline_node(last_node) && !last_node.is_html_br_element() && first_child->is_html_br_element() && !is_inline_node(original_parent))
        first_child->remove();
