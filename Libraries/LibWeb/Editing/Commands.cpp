...
bool command_delete_action(DOM::Document& document, String const&)
{
    ...
    DOM::Node* offset_minus_one_child;
    while (true) {
        ...
        if (offset_minus_one_child && offset_minus_one_child->is_html_anchor_element() && offset_minus_one_child->is_editable()) {
        ...
        if (offset_minus_one_child && !is_block_node(*offset_minus_one_child)
            && !offset_minus_one_child->is_html_br_element() && !offset_minus_one_child->is_html_image_element()) {
    ...
    if (is_block_node(*node)) {
        if (auto* child_node = node->child_at_index(offset - 1)) {
            ...
            block_node_child_is_relevant_type = any_of(Array { HTML::TagNames::br, HTML::TagNames::hr, HTML::TagNames::img }, [&child_element](FlyString const& tag_name) {
                return tag_name == child_element->local_name();
            });
        }
    }
    if ((node->is_text() && offset != 0) || block_node_child_is_relevant_type) {
    ...
    if (offset == 0 && node->index() == 0
        && any_of(Array { HTML::TagNames::li, HTML::TagNames::dt, HTML::TagNames::dd }, [&node_element](FlyString const& tag_name) {
            return tag_name == node_element.local_name();
        })) {
    ...
    if (start_node->child_at_index(start_offset)->is_html_table_element())
        return true;

    // 12. If start node has a child with index start offset − 1, and that child is a table:
    offset_minus_one_child = start_node->child_at_index(start_offset - 1);
    if (offset_minus_one_child && offset_minus_one_child->is_html_table_element()) {
    ...
    // FIXME: 13. If offset is zero; and either the child of start node with index start offset minus one
    //     is an hr, or the child is a br whose previousSibling is either a br or not an inline
    //     node:
    if (false) {
        // FIXME: 1. Call collapse(start node, start offset − 1) on the context object's selection.

        // FIXME: 2. Call extend(start node, start offset) on the context object's selection.

        // FIXME: 3. Delete the selection.

        // FIXME: 4. Call collapse(node, offset) on the selection.

        // 5. Return true.
        return true;
    }

    // FIXME: 14. If the child of start node with index start offset is an li or dt or dd, and that child's
    //     firstChild is an inline node, and start offset is not zero:
    {
        // FIXME: 1. Let previous item be the child of start node with index start offset minus one.

        // FIXME: 2. If previous item's lastChild is an inline node other than a br, call
        //    createElement("br") on the context object and append the result as the last child of
        //    previous item.

        // FIXME: 3. If previous item's lastChild is an inline node, call createElement("br") on the
